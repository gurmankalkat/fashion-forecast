  ...
  const fetchWithRetry = async (url: string, retries = 5, delay = 1000) => {
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(`Failed to fetch: ${response.status}`); // Fixed by adding backticks for template literal
      }
    } catch (error) {
      ...
      (entries) => {
        ...
        if (entry.isIntersecting && !hasFetched && city) {
          const fetchData = async () => {
            ...
            const query = `This is the biggest difference in fashion in ${city} today versus 5 years ago:`; // Fixed by adding backticks for template literal
            const data = await fetchWithRetry(`/api/perform-rag?type=tldr&query=${encodeURIComponent(query)}&city=${encodeURIComponent(city)}`); // Fixed by adding backticks for template literal
            ...
            <div className="mt-6">
              {imageUrl === "Unable to generate image" ? (
                ...
                <img
                  ...
                  className="h-64 w-64 w-full object-cover mt-4"
                />
              )}
            </div>
          </div>
