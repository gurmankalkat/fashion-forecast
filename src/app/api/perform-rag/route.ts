...
export async function GET(req: NextRequest) {
  try {
    ...
    if (queryType === 'tldr') {
      ...
      systemPrompt = "You are a fashion researcher specializing in trend analysis. Your task is to provide a detailed list of max 10 specific trendy clothing items. Avoid general terms and focus on individual items (e.g., 'oversized denim jacket,' 'ribbed knit turtleneck'). Include any relevant materials, colors, or patterns mentioned. Give the list of trending fashion items as a comma-separated string without dashes or bullet points (item 1, item 2, item 3, etc). Do not put a period after the last item.";
